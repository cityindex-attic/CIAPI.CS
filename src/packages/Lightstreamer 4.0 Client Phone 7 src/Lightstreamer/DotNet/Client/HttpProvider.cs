namespace Lightstreamer.DotNet.Client
{
    ...
    internal class HttpProvider
    {
        ...
        protected internal static HttpWebRequest CreateWebRequest(string address, CookieContainer cookies)
        {
            ...
            HttpWebRequest connection = WebRequest.Create(url);
            if (connection == null)
            {
                streamLogger.Debug("Failed connection to " + address);
                throw new IOException("Connection failed");
            }
            connection.set_CookieContainer(cookies);
            connection.set_AllowReadStreamBuffering(false);
            return connection;
        }
        ...
        internal virtual Stream DoHTTP(bool isPost)
        {
            ...
            try
            {
                ...
                CS$1$0000 = response.GetResponseStream();
            }
            catch (Exception e)
            {
                if (response != null)
        ...
        protected internal virtual HttpWebRequest SendPost()
        {
            ...
            connection.set_Method("POST");
            connection.set_ContentType("application/x-www-form-urlencoded");
            MyRequestState state = new MyRequestState {
                request = connection
            };
