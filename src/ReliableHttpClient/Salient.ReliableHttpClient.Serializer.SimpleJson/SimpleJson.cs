...
namespace SimpleJson
{
 ...
 ? Convert.ChangeType(value, type, CultureInfo.InvariantCulture) : value;
            ...
            else
            {
                if (value is IDictionary<string, object>)
                {
...
#else
                        IDictionary dict = (IDictionary) this.CacheResolver.GetNewInstance(genericType);
#endif
                    ...
                    {
                        obj = this.CacheResolver.GetNewInstance(type);
 ...
 typeof(IList).GetTypeInfo().IsAssignableFrom(type.GetTypeInfo())
                    ...
                    Type innerType = type.GetTypeInfo().GenericTypeArguments[0];
                        ...
                        list = (IList) this.CacheResolver.GetNewInstance(genericType);
                        foreach (object o in jsonObject)
